...
public class MultipartParser {
	...
	private String[] extractDispositionInfo(String line) throws IOException {
		...
		int start = _line.indexOf("content-disposition: ");
		int end = _line.indexOf(";");
		if (start == -1 || end == -1) {
			throw new IOException("Content disposition corrupt: " + origline);
		}
		String disposition = _line.substring(start + 21, end).trim();
		if (!disposition.equals("form-data")) {
			throw new IOException("Invalid content disposition: " + disposition);
		}
		// Get the field name
		start = _line.indexOf("name=\"", end); // start at last semicolon
		end = _line.indexOf("\"", start + 7); // skip name=\"
		int startOffset = 6;
		if (start == -1 || end == -1) {
			// Some browsers like lynx don't surround with ""
			// Thanks to Deon van der Merwe, dvdm@truteq.co.za, for noticing
			start = _line.indexOf("name=", end);
			end = _line.indexOf(";", start + 6);
			if (start == -1) {
	...
	private static String extractContentType(String line) {
		...
		return _line.substring(13, end).trim(); // "content-type:" is 13
	}
