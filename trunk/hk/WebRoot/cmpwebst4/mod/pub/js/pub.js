...
function createCenterWindow2(win_id,width,height,title,html,hideEvent,closeBtnFocus){
	createCenterWindow(win_id,width,height,title,html,hideEvent);
	if(closeBtnFocus){
		getObj('popwin_close_btn').focus();
	}
}
...
function tologin(){
	tourl("/login?return_url="+encodeLocalURL());
}
function toreg(){
	tourl("/signup?return_url="+encodeLocalURL());
}
function alertLoginAndReg(){
	createBg();
	var title="请先";
	var html='<div align="center" style="padding-top:30px;"><input type="button" class="btn" style="margin-right:30px;" value="登录" onclick="tologin()"/><input type="button" class="btn split-r" value="注册" onclick="toreg()"/></div>';
	createCenterWindow("loginalert",420,200,title,html,"hideWindow('loginalert');clearBg();");
}