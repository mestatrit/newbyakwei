...
neverModules.modules.autocomplete.prototype = {
  ...
  _hdleClick: function (oElement) {
	...
	var oid=this.getSelectedValue(oElement).oid;
    this._inputValue = text;
    this.returnBox.value = text;
    this._fireCallbackHdle(oid,text, content);
  ...
  */
  ...
  _queryMutiDataSource: function (strQuery) {
    ...
    var text, content, hints, rText, rg,oid; this._formatData=[];
    ...
    for (var i=0; i<a.length; i++) {
      ...
      if (text.match(rg)) {
        ...
        this._formatData.push({text:text, content:content, hints:hints, rText: rText,oid:oid});
      }
    }
    /* reset cache data */
    this._cacheData = [];
  ...
  _formatSingleDataSource: function () {
    ...
    if (a.length==1) {
      strHTML = ''
      + '<div class="out" oid="'+a[0].oid+'" onclick="' +this.name+'._hc(this)" onmouseover="' +this.name+'._tg(this,\'over\')" onmouseout="' +this.name+'._tg(this,\'out\')">' +a[0]+ '</div>';
    } else {
      strHTML = ''
  ...
  _formatMutiDataSource: function () {
    ...
    var text,rText,content,hints,strHTML,oid;
    ...
    for (var i=0; i<a.length; i++) {
	  ...
	  oid=a[i].oid;
      strBuilder.push(''
      + '\n<tr title="' +hints+ '" oid="'+oid+'" uid="' +i+ '" class="out" txt="' +text+ '" content="' +content+ '" onclick="' +this.name+'._hc(this)" onmouseover="' +this.name+'._tg(this,\'over\',\'autot-over\',\'autoc-over\')" onmouseout="' +this.name+'._tg(this,\'out\',\'autot\',\'autoc\')">\n<td class="autot">' +rText+ '</td>\n<td class="autoc">' +content+ '</td>\n</tr>\n');
    }
    ...
    strHTML += "</table>";
    if(this.showFooter){
    	strHTML += this.showFooterHtml;
    }
    if(this.showClose){
    	strHTML += '<div style="position:absolute;bottom: 0px;right:10px;"><a style="text-decoration: underline;" href="javascript:autoComplete.close()">关闭</a></div>'
    }
    return strHTML;
  },
  ...
  */
  ...
  getSelectedValue: function (oElement) {
	...
	var oid=this.useContent?
    oElement.getAttribute("oid"):"";
    return {oid:oid,text: text, content: content};
  },
