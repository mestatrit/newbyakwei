...
<hk:wap title="查询足迹 - 火酷" rm="false" bodyId="thepage">
	...
	<div class="hang even">查询足迹组<br/>
	<hk:form method="get" action="/admin/badge_searchcmpadmingroup.do">
	...
	<c:if test="${fn:length(list)>0}">
		<c:forEach var="g" items="${list}" varStatus="idx"><c:if test="${idx.index%2==0}"><c:set var="clazz_var" value="odd" /></c:if><c:if test="${idx.index%2!=0}"><c:set var="clazz_var" value="even" /></c:if>
			<div class="hang ${clazz_var }">
				${g.name}
				<hk:a href="/admin/badge_op.do?groupId=${g.groupId}&method=${method }">选择</hk:a>
			</div>
		</c:forEach>
		<hk:simplepage2 href="/admin/badge_searchcmpadmingroup.do?name=${enc_name }&method=${method }"/>
	</c:if>
