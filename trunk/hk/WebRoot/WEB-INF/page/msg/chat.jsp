			...
			<c:forEach var="vo" items="${volist}" varStatus="idx"><c:if test="${idx.index%2==0}"><c:set var="clazz_var" value="odd" /></c:if><c:if test="${idx.index%2!=0}"><c:set var="clazz_var" value="even" /></c:if>
				<tr class="${clazz_var}">
					...
					<td>
						...
						${vo.pvtChat.spHtml } 
						<c:if test="${vo.pvtChat.smsSend}"><hk:a clazz="s" href="/msg/chat.do?mainId=${mainId }&sms=1&simplestate=${vo.userSms.sendState }">(<hk:data key="view.sms.simplestate${vo.userSms.sendState}"/>)</hk:a>
						<c:if test="${vo.userSms.sendFail}"><hk:a href="/msg/send_resendsms.do?mainId=${mainId}&smsmsgId=${vo.userSms.msgId }"><hk:data key="view.resendsms"/></hk:a></c:if> 
						</c:if>
						<hk:a href="/msg/pvt_delchat.do?chatId=${vo.pvtChat.chatId}&mainId=${mainId }" page="true">删</hk:a>
					</td>
				</tr>
			</c:forEach>
		</tbody></table>
		<hk:simplepage clazz="page" href="/msg/chat.do?mainId=${main.mainId}&sms=${sms }" />
		<div class="hang"><hk:a href="/msg/pvt_del.do?mainId=${main.mainId}">删除本对话录</hk:a></div>
	</c:if>
	<c:if test="${fn:length(volist)==0}">没有私信数据显示</c:if>
	<div class="hang"><hk:a href="/msg/chat.do?mainId=${main.mainId}">返回</hk:a></div>
	<c:if test="${simplestate!=null}">
	<div class="hang even">
		<hk:data key="view.sms.simplestateex${simplestate}"/>
	</div>
	</c:if>
