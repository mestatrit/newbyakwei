<%@ page language="java" pageEncoding="UTF-8"%><%@page import="com.hk.web.util.HkWebConfig"%><%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%><%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %><%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%><%@ taglib uri="/WEB-INF/waphk.tld" prefix="hk"%><hk:wap title="${box.name}开箱记录 - 火酷" rm="false" bodyId="thepage">
	...
	<hk:a href="/box/box_awarduserlist.do?boxId=${boxId}&repage=${repage }">中奖</hk:a>|
	<hk:a href="/box/box_userlist.do?boxId=${boxId}&repage=${repage }" clazz="nn">所有</hk:a>
	</hk:rmBlankLines>
	</div>
	<c:if test="${fn:length(list)>0}">
		...
		<tr class="${clazz_var }"><td>
				...
				<c:if test="${b.userId!=serviceUserId}">
					<c:set var="mobile" value="${b.user.userOtherInfo.mobile}" /><c:set var="mobileBind" value="${b.user.userOtherInfo.mobileBind}" />
	...
	<hk:simplepage href="/box/box_userlist.do?boxId=${boxId}" returndata="返回" returnhref="/box/box_list.do?page=${repage}"/>
