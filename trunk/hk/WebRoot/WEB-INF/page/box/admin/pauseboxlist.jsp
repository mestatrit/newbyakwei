...
<hk:wap title="停止的宝箱 - 火酷" rm="false" bodyId="thepage">
	<jsp:include page="../../inc/top.jsp"></jsp:include>
	<div class="hang even">停止的宝箱</div>
	<div class="hang">
		<hk:form method="get" action="/box/admin/adminbox_pauseboxlist.do">
		...
		<table class="list" cellpadding="0" cellspacing="0"><tbody>
			<c:forEach var="b" items="${list}" varStatus="idx"><c:if test="${idx.index%2==0}"><c:set var="clazz_var" value="odd" /></c:if><c:if test="${idx.index%2!=0}"><c:set var="clazz_var" value="even" /></c:if>
				<tr class="${clazz_var }"><td>
					<hk:a href="/box/op/op_getbox.do?boxId=${b.boxId}&t=5&fromadmin=1" page="true">
						${b.name} &nbsp;${b.totalCount-b.openCount}/${b.totalCount}
					</hk:a>
				</td></tr>
			</c:forEach>
		</tbody></table>
		<hk:simplepage href="/box/admin/adminbox_pauseboxlist.do?name=${enc_name}" />
	</c:if>
