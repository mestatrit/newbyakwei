...
onload= function pageLoadHdle() {
	...
	autoComplete.showFooterHtml='<div style="padding-left:10px"><a style="text-decoration: underline;" href="<%=path %>/e/op/orderform_help.do?companyId=${companyId}&oid=${oid}&return_url=${return_url}"><hk:data key="view.company.product.notfound"/></a></div>';
}
</script>
</c:set>
  ...
  width:50%;
	...
	<div class="mod_primary">
		...
		<div class="mod_primary_l">
			...
			<div>
				<h3 class="title4">产品列表</h3>
				<table class="infotable cellinfo rowclick" cellpadding="0" cellspacing="0">
					<tr class="tr-title">
						<th width="100px">编号</th>
						<th width="200px">名称</th>
						<th width="100px">缩写</th>
						<th width="100px">价格</th>
						<th width="100px"></th>
					</tr>
					<c:forEach var="p" items="${productlist}">
						<tr class="tr-line" onclick="additem(${p.productId },1)" onmouseout="this.className='tr-line';" onmouseover="this.className='tr-line bg2';">
							<td>${p.pnum }</td>
							<td>${p.name }</td>
							<td>${p.shortName }</td>
							<td>￥${p.money }</td>
							<td>
							<a href="javascript:void(0)">点击添加</a>
							<span id="op${p.productId }" class="error"></span>
							</td>
						</tr>
					</c:forEach>
				</table>
				<div>
				<hk:page midcount="10" url="/e/op/orderform_productlist.do?companyId=${companyId}&oid=${oid }&sortId=${sortId }&return_url=${return_url }"/>
				<div class="clr"></div>
				</div>
			</div>
			<div>
				<div class=""><br/>
				<div class="h3s pad">已添加的产品</div>
				</div>
				<div id="itemlist">
				</div>
...
function addfirstitem(event){
	if(dataSource.length>0){
		additem(dataSource[0].oid,0);
		autoComplete.close();
	}
	return false;
}
function additem(pid,flg){
	if(flg==1){
		setHtml('op'+pid,'添加中 ... ...');
	}
	else{
