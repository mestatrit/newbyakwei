...
<c:set var="html_title" scope="request"><hk:data key="view.user_friend.title" arg0="${user.nickName}" /></c:set>
	...
	<table class="frame-3" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td class="mid">
				<div class="mid_con">
					<div class="mod">
						<c:set var="nav_2_short_content" scope="request"><a href="<%=path %>/friend.do?userId=${userId }" class="nav-a"><hk:data key="view.user_friend.title" arg0="${user.nickName}" /></a></c:set>
						<jsp:include page="../inc/nav-2-short.jsp"></jsp:include>
					</div>
					<div class="mod">
						<c:set var="page_url" scope="request"><%=path%>/friend.do?userId=${userId}</c:set>
