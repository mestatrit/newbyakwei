...
<ul class="ulblock2">
	<c:forEach var="vo" items="${item_user_refvolist}" varStatus="idx">
		<li idx="${vo.tbItemUserRef.oid }" <c:if test="${idx.index%2!=0}">class="even"</c:if>>
			<div class="item_img"><a href="${ctx_path }/tb/item?itemid=${vo.tbItemUserRef.itemid}"><img src="${vo.tbItemUserRef.tbItem.pic_url }_60x60.jpg" /></a></div>
			<div class="content"><a href="${ctx_path }/tb/item?itemid=${vo.tbItemUserRef.itemid}">${vo.tbItemUserRef.tbItem.title}</a><br/>
			${vo.tbItemUserRef.tbItemCmt.content }</div>
