...
<div class="hcenter divrow" style="width:750px">
	<div class="mod">
		<div class="mod_content">
			<h1><span class="f"><hk:data key="epp.user.signup"/></span></h1>
			<br/>
			<form id="regfrm" method="post" onsubmit="return subregfrm(this.id)" action="<%=path %>/epp/web/user_reg.do" target="hideframe">
				...
				<table class="nt all" cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<td>
							<input type="text" name="code" class="text_yzm" maxlength="4" size="4" style="width: 50px"/>
					...
					<tr>
						<td colspan="2">
							<hk:submit value="epp.reg.submit.user" clazz="btn split-r" res="true"/>
							<hk:submit value="epp.reg.submit.cmporg" clazz="btn" res="true" onclick="setorgstatus()"/>
						</td>
					</tr>
				</table>
			</form>
...
function regok(error,error_msg,respValue){
	...
	if(tocreateorg==1){
		tourl('<%=path %>/epp/web/user_adminorg.do?companyId=${companyId}');
	}
	else{
