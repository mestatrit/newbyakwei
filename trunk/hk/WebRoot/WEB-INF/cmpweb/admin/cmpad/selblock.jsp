...
<div class="hcenter" style="width: 820px;">
	...
	<div class="mgrright">
		<div class="mod">
		<div class="mod_title">企业广告推荐</div>
		<div class="mod_content">
			<c:if test="${cmpAdBlock!=null}">
			<div class="divrow">
			已推荐到 <span class="b">${cmpPageBlock.name }</span> <a href="javascript:deladblock(${cmpAdBlock.oid })">取消推荐</a>
			</div>
			</c:if>
			<c:if test="${fn:length(blocklist)>0}">
				<table class="nt" cellpadding="0" cellspacing="0">
					<c:forEach var="block" items="${blocklist}">
						<c:if test="${block.cmpPageMod.modAd && cmpAdBlock.blockId!=block.blockId}">
...
function deladblock(oid){
	$.ajax({
		type:"POST",
		url:"<%=path%>/epp/web/op/webadmin/cmpad_deladblock.do?companyId=${companyId}&oid="+oid,
		cache:false,
    	dataType:"html",
...
function selblock(blockId){
	$.ajax({
		type:"POST",
		url:"<%=path%>/epp/web/op/webadmin/cmpad_selblock.do?ch=1&companyId=${companyId}&adid=${adid}&blockId="+blockId,
		cache:false,
    	dataType:"html",
		success:function(data){
			if(data=="1"){
