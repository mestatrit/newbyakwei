...
<div class="hcenter" style="width: 820px;">
	...
	<div class="mgrright">
		<div class="mod">
			<div class="mod_title">奖品图片
			<a class="more" href="<%=path %>/epp/web/op/webadmin/box_view.do?companyId=${companyId }&boxId=${boxId}">返回</a>
			</div>
			<div class="mod_content">
				<div class="divrow">
					<form method="post" enctype="multipart/form-data" action="<%=path %>/epp/web/op/webadmin/box_uploadpic.do" target="hideframe">
			...
			<div>
				<c:set var="page_url" scope="request"><%=path %>/epp/web/op/webadmin/box_selprizepic.do?companyId=${companyId}&boxId=${boxId}&prizeId=${prizeId}&navoid=${navoid }</c:set>
...
function uploadok(error,msg,v){
	tourl("<%=path %>/epp/web/op/webadmin/box_view.do?companyId=${companyId}&boxId=${boxId}&navoid=${navoid }");
}
function selpic(photoId){
	$.ajax({
		type:"POST",
		url:"<%=path%>/epp/web/op/webadmin/box_selpic.do?companyId=${companyId}&boxId=${boxId}&prizeId=${prizeId}&photoId="+photoId,
		cache:false,
    	dataType:"html",
