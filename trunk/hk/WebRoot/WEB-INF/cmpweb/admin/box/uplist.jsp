...
<div class="hcenter" style="width: 820px;">
	...
	<div class="mgrright">
		<div class="mod">
			<div class="mod_title">${box.name }
				<a class="more" href="<%=path %>/epp/web/op/webadmin/box_view.do?companyId=${companyId}&boxId=${boxId}&navoid=${navoid }">返回</a>
			</div>
			<div class="mod_content">
				<div class="divrow">
					<form method="get" action="<%=path %>/epp/web/op/webadmin/box_uplist.do">
						<hk:hide name="companyId" value="${companyId}"/>
						<hk:hide name="boxId" value="${boxId}"/>
						<hk:hide name="navoid" value="${navoid}"/>
						序列号：<hk:text name="prizeNum" value="${prizeNum}" clazz="text2"/>
						暗号：<hk:text name="prizePwd" value="${prizePwd}" clazz="text2"/>
				...
				<c:if test="${fn:length(list)>0}">
					<c:forEach var="n" items="${list}">
						<div class="divrow bdtm" onmouseover="this.className='divrow bdtm bg2'" onmouseout="this.className='divrow bdtm'">
							...
							<div class="f_l">
								<a href="javascript:setused(${n.sysId })">设为已兑换</a>
							</div>
							<div class="clr"></div>
						</div>
					</c:forEach>
			...
			<div>
				<c:set var="page_url" scope="request"><%=path %>/epp/web/op/webadmin/box_uplist.do?companyId=${companyId}&boxId=${boxId}&navoid=${navoid }&prizeNum=${prizeNum}&prizePwd=${prizePwd}</c:set>
...
function setused(oid){
	$.ajax({
		type:"POST",
		url:"<%=path%>/epp/web/op/webadmin/box_setused.do?companyId=${companyId}&oid="+oid,
		cache:false,
    	dataType:"html",
