...
<c:if test="${fn:length(list)>0}">
	<c:forEach var="coupon" items="${list}" varStatus="idx"><c:if test="${idx.index%2==0}"><c:set var="clazz_var" value="odd" /></c:if><c:if test="${idx.index%2!=0}"><c:set var="clazz_var" value="even" /></c:if>
	<div class="row ${clazz_var }">
		<a href="<%=path %>/epp/web/coupon_wapview.do?companyId=${companyId}&couponId=${coupon.couponId }&navId=${navId}">${coupon.name }</a>
	</div>
	</c:forEach>
</c:if>
<div class="row">
	<hk:simplepage2 href="/epp/web/coupon_wap.do?companyId=${companyId}&navId=${navId}"/>
</div>
