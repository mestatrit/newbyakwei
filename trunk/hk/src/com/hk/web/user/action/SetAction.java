...
public class SetAction extends BaseAction {
	 ...
	 * @throws Exception
	 */
	public String setlesscharfilter(HkRequest req, HkResponse resp)
			throws Exception {
		User loginUser = this.getLoginUser(req);
	 ...
	 * @throws Exception
	 */
	public String toSetNickName(HkRequest req, HkResponse resp)
			throws Exception {
	...
	public String toSetMsn(HkRequest req, HkResponse resp) {
		if (req.getAttribute("info") == null) {
			UserBindInfo info = this.userService.getUserBindInfo(this
	...
	public String setmsnweb(HkRequest req, HkResponse resp) {
		String msn = req.getString("msn");
		if (isEmpty(msn) || msn.length() > 50) {
			return this.initError(req, Err.MSN_ERROR, -1, null, "msn",
					"onmsnerror", null);
		}
		long userId = this.getLoginUser(req).getUserId();
		UserBindInfo info = this.userService.getUserBindInfo(userId);
		...
		try {
			...
			return this.initError(req, Err.MSN_ALREADY_EXIST, -1, null, "msn",
					"onmsnerror", null);
		}
	}
	...
	public String toSetInfo(HkRequest req, HkResponse resp) {
		...
		req.setAttribute("user", this.userService.getUser(this
				.getLoginUser(req).getUserId()));
	...
	public String setinfoweb(HkRequest req, HkResponse resp) {
		...
		UserCard userCard = this.userCardService.getUserCard(loginUser
				.getUserId());
		if (userCard != null) {
	...
	public String toSetHead(HkRequest req, HkResponse resp) throws Exception {
		User user = this.userService
				.getUser(this.getLoginUser(req).getUserId());
		req.setAttribute("user", user);
	...
	public String setpwdweb(HkRequest req, HkResponse resp) {
		...
		long userId = this.getLoginUser(req).getUserId();
		if (DataUtil.isEmpty(oldPwd)) {
			return this.initError(req, Err.PASSWORD_DATA_ERROR, -1, null,
					"pwd", "onpwderror", null);
		}
		int code = UserValidate.validatePassword(newPwd);
		if (code != Err.SUCCESS) {
			return this.initError(req, code, -1, null, "pwd", "onpwderror",
					null);
		}
		if (this.userService.updatePwd(userId, oldPwd, newPwd)) {
		...
		return this.initError(req, Err.PASSWORD_OLD_ERROR, -1, null, "pwd",
				"onpwderror", null);
	}
	 ...
	 * @throws Exception
	 */
	public String tosetmobile2web(HkRequest req, HkResponse resp)
			throws Exception {
		int refresh = req.getInt("refresh");
		long userId = this.getLoginUser(req).getUserId();
		Randnum o = null;
	...
	public String toChgMobile(HkRequest req, HkResponse resp) {
		...
		if (o == null) {
			...
			catch (CreateRandnumException e) {// 暂时忽略此异常
			}
		}
		req.setAttribute("ran", o);
		req.setAttribute("userOtherInfo", userOtherInfo);
	 ...
	 * @throws Exception
	 */
	public String tosetemailweb(HkRequest req, HkResponse resp)
			throws Exception {
		UserOtherInfo info = this.userService.getUserOtherInfo(this
				.getLoginUser(req).getUserId());
		req.setAttribute("info", info);
	...
	public String setEmail(HkRequest req, HkResponse resp) {
		...
		try {
			UserOtherInfo info = this.userService.getUserOtherInfo(this
					.getLoginUser(req).getUserId());
			// 如果是同一个email地址而且已经认证，就不发送email
			if (info.getEmail().equals(email) && info.isAuthedMail()) {
				return "r:/user/set/set.do";
			}
			this.userService.updateEmail(userId, email);
			UserMailAuth o = this.userMailAuthService.createUserMailAuth(info
					.getUserId());
			if (info.isAuthedMail()) {
				ScoreLog scoreLog = ScoreLog.create(userId,
						HkLog.CANCELVALIDATEEMAIL, 0, -ScoreConfig
								.getCancelValidateEmail());
			...
			String content = req.getText("func.mail.validateemail.content", o
					.getAuthcode());
	...
	public String setemailweb(HkRequest req, HkResponse resp) {
		...
		int code = UserOtherInfo.validateEmail(email);
		if (code != Err.SUCCESS) {
			return this.initError(req, code, -1, null, "email", "onemailerror",
					null);
		}
		try {
			UserOtherInfo info = this.userService.getUserOtherInfo(this
			...
			// 发送email认证码
			String title = req.getText("func.mail.validateemail.title");
			String content = req.getText("func.mail.validateemail.contentweb",
					o.getAuthcode());
			this.mailUtil.sendHtmlMail(email, title, content);
			req.setSessionText("func.mail.sendauthcodemailok");
			...
			return this.initError(req, Err.EMAIL_ALREADY_EXIST, -1, null,
					"email", "onemailerror", null);
		}
		catch (MessagingException e) {
			e.printStackTrace();
			return this.initError(req, Err.EMAIL_SEND_ERROR, -1, null, "email",
					"onemailerror", null);
		}
	}
	 ...
	 * @throws Exception
	 */
	public String tosetprotectweb(HkRequest req, HkResponse resp)
			throws Exception {
		long userId = this.getLoginUser(req).getUserId();
		UserProtect userProtect = (UserProtect) req.getAttribute("userProtect");
		if (userProtect == null) {
	...
	public String setprotectweb(HkRequest req, HkResponse resp) {
		...
		if (code != Err.SUCCESS) {
			return this.initError(req, code, -1, null, "pwd", "onproerror",
					null);
		}
		this.userService.updateUserProtect(userId, pconfig, pvalue);
	...
	public String abolishUser(HkRequest req, HkResponse resp) {
		...
		log.info("abolishUser ownnerId[ " + this.getLoginUser(req).getUserId()
				+ " ] input [ " + input + " ]");
		User user = this.userService.abolishUser(this.getLoginUser(req)
				.getUserId(), input, pwd);
		if (user != null) {
	...
	public String uploadheadpic(HkRequest req, HkResponse resp) {
		...
		try {
			...
			return this.onSuccess(req, picurl + ";" + width + ";" + height,
					"uploadpic");
		}
		catch (ImageException e) {
