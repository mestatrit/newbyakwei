 ...
 */
@Component("/followed")
public class FollowedAction extends BaseAction {
	@Autowired
	...
	public String execute(HkRequest req, HkResponse resp) throws Exception {
		...
		List<Followed> followedlist = this.followService.getFollowedList(
				userId, page.getBegin(), page.getSize() + 1);
		this.processListForPage(page, followedlist);
		List<Long> userIdList = new ArrayList<Long>();
		for (Followed f : followedlist) {
			userIdList.add(f.getFollowingUserId());
		}
		Map<Long, User> usermap = this.userService.getUserMapInId(userIdList);
		List<User> duserlist = new ArrayList<User>();
		for (Followed f : followedlist) {
			duserlist.add(usermap.get(f.getFollowingUserId()));
		}
		...
		return this.getWeb3Jsp("user/followedlist.jsp");
	}
}