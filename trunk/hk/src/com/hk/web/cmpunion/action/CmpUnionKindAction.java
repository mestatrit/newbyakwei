...
public class CmpUnionKindAction extends CmpUnionBaseAction {
	...
	public String list(HkRequest req, HkResponse resp) {
		...
		List<CmpUnionKind> kindlist = this.cmpUnionService
				.getCmpUnionKindListByUid(uid, page.getBegin(),
						page.getSize() + 1);
	...
	public String cmdlist(HkRequest req, HkResponse resp) {
		...
		List<CmpUnionCmdKind> kindlist = this.cmpUnionService
				.getCmpUnionCmdKindListByUid(uid, page.getBegin(), page
						.getSize() + 1);
		this.processListForPage(page, kindlist);
	 ...
	 * @return
	 */
	private String listKind(HkRequest req, HkResponse resp,
			CmpUnionKind cmpUnionKind) {
		SimplePage page = req.getSimplePage(20);
		List<CmpUnionKind> kindlist = this.cmpUnionService
				.getCmpUnionKindListByUid(cmpUnionKind.getUid(), cmpUnionKind
						.getKindId(), page.getBegin(), page.getSize() + 1);
		this.processListForPage(page, kindlist);
		req.setAttribute("kindlist", kindlist);
	 ...
	 * @return
	 */
	private String listProduct(HkRequest req, HkResponse resp,
			CmpUnionKind cmpUnionKind) {
		long kindId = req.getLongAndSetAttr("kindId");
		SimplePage page = req.getSimplePage(20);
		List<CmpProduct> productlist = this.cmpProductService
				.getCmpProductListByCmpUnionKindId(kindId, page.getBegin(),
						page.getSize() + 1);
	 ...
	 * @return
	 */
	private String listCmp(HkRequest req, HkResponse resp,
			CmpUnionKind cmpUnionKind) {
		long kindId = req.getLongAndSetAttr("kindId");
		SimplePage page = req.getSimplePage(20);
		List<Company> companylist = this.companyService
				.getCompanyListByUnionKindId(kindId, page.getBegin(), page
						.getSize() + 1);
