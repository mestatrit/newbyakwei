...
public class AdminAction extends BaseAction {
	...
	public String addgd(HkRequest req, HkResponse resp) throws Exception {
		...
		if (count != 0) {
			...
			User user = this.userService.getUser(userId);
			UserTool userTool = this.userService
					.checkUserTool(user.getUserId());
			userTool.setGroundCount(userTool.getGroundCount() + count);
			this.userService.updateuserTool(userTool);
			String s = "修改地皮成功，" + user.getNickName() + "当前的地皮数量为"
					+ userTool.getGroundCount();
	...
	public String gdlist(HkRequest req, HkResponse resp) throws Exception {
		...
		if (DataUtil.isEmpty(nickName)) {
			...
			if (user != null) {
				UserTool userTool = this.userService.getUserTool(user
						.getUserId());
				if (userTool == null) {
					userTool = UserTool.createDefault(user.getUserId());
				}
	...
	public String hkblog(HkRequest req, HkResponse resp) throws Exception {
		...
		SimplePage page = req.getSimplePage(size);
		List<AdminHkb> list = this.userService.getAdminHkbList(userId, page
				.getBegin(), size);
		page.setListSize(list.size());
	 ...
	 * @throws Exception
	 */
	public String deffollowuser(HkRequest req, HkResponse resp)
			throws Exception {
		SimplePage page = req.getSimplePage(size);
		List<DefFollowUser> list = this.userService.getDefFollowUserList(page
				.getBegin(), size);
		page.setListSize(list.size());
		req.setAttribute("list", list);
	 ...
	 * @throws Exception
	 */
	public String adddeffollowuser(HkRequest req, HkResponse resp)
			throws Exception {
		String nickName = req.getString("nickName");
		User user = this.userService.getUserByNickName(nickName);
		if (user != null) {
	 ...
	 * @throws Exception
	 */
	public String deldeffollowuser(HkRequest req, HkResponse resp)
			throws Exception {
		long userId = req.getLong("userId");
	...
	public String setcmpfunc(HkRequest req, HkResponse resp) {
		...
		return "r:/admin/admin_seluserforcmpfunc.do?ch=1&nickName="
				+ DataUtil.urlEncoder(user.getNickName());
	}
}