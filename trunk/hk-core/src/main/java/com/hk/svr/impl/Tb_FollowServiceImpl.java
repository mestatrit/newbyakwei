...
public class Tb_FollowServiceImpl implements Tb_FollowService {
	...
	public void createTb_Follow(Tb_Follow tbFollow, boolean follow_to_sina) {
		...
		if (query.getObjectEx(Tb_Follow.class, "userid=? and friendid=?",
				new Object[] { tbFollow.getUserid(), tbFollow.getFriendid() }) == null) {
	...
	public void createTb_Followed(Tb_Followed tbFollowed) {
		...
		Query query = this.manager.createQuery();
		if (query
				.getObjectEx(Tb_Followed.class, "userid=? and fansid=?",
						new Object[] { tbFollowed.getUserid(),
								tbFollowed.getFansid() }) == null) {
	...
	public void deleteTb_Follow(Tb_Follow tbFollow) {
		this.manager.createQuery().deleteById(Tb_Follow.class,
				tbFollow.getOid());
	}

	public void deleteTb_Followed(Tb_Followed tbFollowed) {
		this.manager.createQuery().deleteById(Tb_Followed.class,
				tbFollowed.getOid());
	}

	public Tb_Follow getTb_FollowByUseridAndFriendid(long userid, long friendid) {
		Query query = this.manager.createQuery();
		return query.getObjectEx(Tb_Follow.class, "userid=? and friendid=?",
				new Object[] { userid, friendid });
	}

	public List<Tb_Follow> getTb_FollowListByUserid(long userid,
			boolean buildFollow, int begin, int size) {
		Query query = this.manager.createQuery();
		return query.listEx(Tb_Follow.class, "userid=?",
				new Object[] { userid }, "oid desc", begin, size);
	}

	public Tb_Followed getTb_FollowedByUseridAndFriendid(long userid,
			long fansid) {
		Query query = this.manager.createQuery();
		return query.getObjectEx(Tb_Followed.class, "userid=? and fansid=?",
	...
	public List<Tb_Followed> getTb_FollowedListByUserid(long userid,
		...
		Query query = this.manager.createQuery();
		return query.listEx(Tb_Followed.class, "userid=?",
				new Object[] { userid }, "oid desc", begin, size);
	}
	...
	public int countTb_FollowByUserid(long userid) {
		Query query = this.manager.createQuery();
		return query
				.count(Tb_Follow.class, "userid=?", new Object[] { userid });
	}

	public int countTb_FollowedByUserid(long userid) {
		Query query = this.manager.createQuery();
		return query.count(Tb_Followed.class, "userid=?",
				new Object[] { userid });
	}

	@Override
	public List<Long> getTb_FollowedFansidListByUserid(long userid) {
		List<Tb_Followed> list = this.getTb_FollowedListByUserid(userid, false,
				0, -1);
