...
public class CmpCheckInProcessor {
	 ...
	 *         2010-4-23
	 */
	public List<CmpCheckInUserLog> getCmpCheckInUserLogListByPcityId(
			int pcityId, boolean buildUser, boolean buildCompany, int begin,
			int size) {
		List<CmpCheckInUserLog> list = this.cmpCheckInService
				.getEffectCmpCheckInUserLogListByPcityId(pcityId, begin, size);
		if (buildUser) {
	...
	public List<CmpCheckInUser> getCmpCheckInUserListByUserId(long userId,
			boolean buildUser, boolean buildCompany, int begin, int size) {
		List<CmpCheckInUser> list = this.cmpCheckInService
				.getCmpCheckInUserListByUserId(userId, begin, size);
		if (buildUser) {
			List<Long> idList = new ArrayList<Long>();
	...
	public List<CmpCheckInUser> getCmpCheckInUserListByCompanyId(
			long companyId, boolean buildUser, int begin, int size) {
		List<CmpCheckInUser> list = this.cmpCheckInService
				.getCmpCheckInUserListByCompanyId(companyId, begin, size);
		if (buildUser) {
			List<Long> idList = new ArrayList<Long>();
			for (CmpCheckInUser o : list) {
				idList.add(o.getUserId());
	 ...
	 *         2010-4-23
	 */
	public List<CmpCheckInUserLog> getFriendCmpCheckInUserLog(long userId,
			boolean buildUser, boolean buildCompany, int size) {
		List<Long> idList = this.followService
				.getFollowFriendIdListByUserId(userId);
		List<UserLastCheckIn> olist = this.cmpCheckInService
		...
		List<CmpCheckInUserLog> loglist = this.cmpCheckInService
				.getCmpCheckInUserLogListInId(list);
		if (buildCompany) {
			list.clear();
	...
	public CheckInResult checkIn(CmpCheckInUserLog cmpCheckInUserLog,
			boolean forceInvalid, Company company, String ip) {
		boolean _forceInvalid = forceInvalid;
		UserDateCheckInCmp userDateCheckInCmp = this.cmpCheckInService
				.getUserDateCheckInCmp(cmpCheckInUserLog.getUserId());
		...
		CheckInResult checkInResult = this.cmpCheckInService.checkIn(
				cmpCheckInUserLog, _forceInvalid, company);
	...
	public int checkMayor(long userId, Company company, String ip,
		...
		int result = this.cmpCheckInService.checkMayor(userId, company, ip,
				forceMayor);
	 ...
	 * 当成为新地主时，记录动态
	 */
	private void onCheckMayored(int result, long userId, long old_mayorId,
			Company company, String ip) {
		if (result == 1) {
			Feed feed = new Feed();
	...
	private void processCompanyUserStatus(CmpCheckInUserLog cmpCheckInUserLog) {
		CompanyUserStatus companyUserStatus = this.companyService
				.getCompanyUserStatus(cmpCheckInUserLog.getCompanyId(),
						cmpCheckInUserLog.getUserId());
		if (companyUserStatus != null && companyUserStatus.isDone()) {
			return;
		}
		this.companyService.createCompanyUserStatus(cmpCheckInUserLog
				.getCompanyId(), cmpCheckInUserLog.getUserId(),
				CompanyUserStatus.OK_FLG);
	}

	private void updateCompanyCheckInCount(CheckInResult checkInResult,
			CmpCheckInUserLog cmpCheckInUserLog) {
		if (checkInResult.isCheckInSuccess()) {
			this.companyService.addCheckInCount(cmpCheckInUserLog
					.getCompanyId(), 1);
		}
	}

	private void processUserLastCheckIn(CheckInResult checkInResult,
			CmpCheckInUserLog cmpCheckInUserLog) {
		if (checkInResult.isCheckInSuccess()) {
			UserLastCheckIn userLastCheckIn = this.cmpCheckInService
					.getUserLastCheckIn(cmpCheckInUserLog.getUserId());
	...
	private void createLaba(CmpCheckInUserLog cmpCheckInUserLog) {
		if (cmpCheckInUserLog.isEffective()) {
			...
			String content = "我在{[" + companyId + ","
					+ DataUtil.toTextRow(company.getName()) + "}";
	...
	private void processAfterCheckIn(CheckInResult checkInResult,
			CmpCheckInUserLog cmpCheckInUserLog, Company company, String ip) {
		...
		this.handleEquipmentProcessor.processEquipment(userId, companyId,
				ctxAttributeMap);
		EquipmentMsg equipmentMsg = (EquipmentMsg) ctxAttributeMap
				.get(EquipmentStatus.EQUIPMENTMSG_ATTR);
	...
	private int getAddPoints(CmpCheckInUserLog cmpCheckInUserLog,
		...
		int userCount = this.cmpCheckInService
				.countEffectCmpCheckInUserLogByCompanyIdAndUserId(companyId,
						userId);
		boolean firstCheckIn = false;// 初次报到
		if (userCount == 1) {
			firstCheckIn = true;
		}
		if (firstCheckIn) {// 第一次报到
			return CheckInPointConfig.getFirstCheckIn() * beishu;
		}
		// 用户当天的报到次数
		int userCountDate = this.cmpCheckInService
				.countEffectCmpCheckInUserLogByCompanyIdAndUserId(companyId,
						userId, new Date());
		if (userCountDate == 1) {// 当天第一次报到
	...
	private void addPoints(CheckInResult checkInResult,
			CmpCheckInUserLog cmpCheckInUserLog,
			Map<String, Object> ctxAttributeMap) {
		int add = 0;
		if (checkInResult.isCheckInSuccess()
				&& this.canAddPoints(ctxAttributeMap)) {
			add = this.getAddPoints(cmpCheckInUserLog, ctxAttributeMap);
			if (add > 0) {
				this.userService.addPoints(cmpCheckInUserLog.getUserId(), add);
				this.cmpCheckInService.addUserCmpPoints(cmpCheckInUserLog
						.getUserId(), cmpCheckInUserLog.getCompanyId(), add);
			}
		}
	}

	private boolean canDoublePoints(Map<String, Object> ctxAttributeMap) {
		Boolean v = (Boolean) ctxAttributeMap
				.get(EquipmentStatus.CAN_GET_DOUBLE_POINTS);
	...
	private boolean canAddPoints(Map<String, Object> ctxAttributeMap) {
		Boolean v = (Boolean) ctxAttributeMap
				.get(EquipmentStatus.CAN_NOT_GET_POINTS);
		if (v != null && v.booleanValue()) {
			return false;
		}
		return true;
