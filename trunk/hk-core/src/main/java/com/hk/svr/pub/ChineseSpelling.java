...
public class ChineseSpelling {
	...
	private static int getCnAscii(char cn) {
		...
		if (bytes.length == 2) { // 中文字符
			...
			// System.out.println("256 * hightByte + lowByte=" + (256 *
			// hightByte + lowByte));
	...
	public static String getShortSpell(String cnStr) {
		char[] ch = cnStr.toCharArray();
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < ch.length; i++) {
			String p = ChineseSpelling.getFullSpell(String.valueOf(ch[i]));
			if (p.length() > 0) {
	...
	public static void main(String[] args) {
		String str = "中国人袁伟，哈哈哈哈哈哈";
		System.out.println(ChineseSpelling.getFullSpell(str));
		System.out.println(ChineseSpelling.getShortSpell(str));
	}
}