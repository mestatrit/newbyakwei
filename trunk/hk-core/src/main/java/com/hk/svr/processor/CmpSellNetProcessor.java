...
public class CmpSellNetProcessor {
	...
	public List<CmpSellNet> getCmpSellNetListByCompanyIdEx(long companyId,
			String name, long kindId, boolean buildKind, int begin, int size) {
		List<CmpSellNet> list = this.cmpSellNetService
				.getCmpSellNetListByCompanyIdEx(companyId, name, kindId, begin,
						size);
		if (buildKind) {
	...
	public List<CmpSellNet> getCmpSellNetListByCompanyId(long companyId,
			boolean buildKind, int begin, int size) {
		List<CmpSellNet> list = this.cmpSellNetService
				.getCmpSellNetListByCompanyId(companyId, begin, size);
		if (buildKind) {
			this.buildCmpSellNetKind(list);
		}
		return list;
	...
	public List<CmpSellNet> getCmpSellNetListByCompanyId(long companyId,
			long kindId, boolean buildKind, int begin, int size) {
		List<CmpSellNet> list = this.cmpSellNetService
				.getCmpSellNetListByCompanyId(companyId, kindId, begin, size);
		if (buildKind) {
			this.buildCmpSellNetKind(list);
		}
		return list;
	...
	private void buildCmpSellNetKind(List<CmpSellNet> list) {
		...
		Map<Long, CmpSellNetKind> map = this.cmpSellNetService
				.getCmpSellNetKindMapInId(idList);
		for (CmpSellNet o : list) {
