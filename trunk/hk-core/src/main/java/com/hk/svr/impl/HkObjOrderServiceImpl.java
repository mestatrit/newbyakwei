...
public class HkObjOrderServiceImpl implements HkObjOrderService {
	...
	public boolean updateHkObjOrder(HkObjOrder hkObjOrder)
			throws NoEnoughMoneyException, SmallerThanMinMoneyException {
		Company company = this.companyService.getCompany(hkObjOrder
				.getHkObjId());
		HkObjOrder o = this.getHkObjOrder(hkObjOrder.getOid());
		if (DataUtil.isSameDay(new Date(), o.getUtime())) {
			return false;
		}
		if (o.getKind() != hkObjOrder.getKind()
				|| o.getCityId() != hkObjOrder.getCityId()) {
		...
		if (amount > 0) {// 如果设置竞排价格大于当前价格，需要从余额中扣除当天差价
			if (company.getMoney() < amount) {// 余额不足扣除当前则异常抛出
				throw new NoEnoughMoneyException("no enough money [ "
						+ company.getMoney() + " , " + hkObjOrder.getMoney()
						+ " ]");
			}
		...
		if (hkObjOrder.getPday() != o.getPday()
				|| hkObjOrder.getMoney() != o.getMoney()) {
			// 先判断是竞价是否改变，
			if (hkObjOrder.getMoney() != o.getMoney()) {
				// 如果竞价改变判断是否低于最低竞价，
				int minmoney = 0;
				HkObjOrderDef hkObjOrderDef = this.orderDefService
						.getHkObjOrderDef(hkObjOrder.getKind(), company
								.getKindId(), hkObjOrder.getCityId());
		...
		query.update(HkObjOrder.class, "oid=?", new Object[] { hkObjOrder
				.getOid() });
		return true;
	}
	...
	private boolean isDuplicate(HkObjOrder hkObjOrder) {
		Query query = manager.createQuery();
		if (query.count(HkObjOrder.class, "hkobjid=? and kind=? and cityid=?",
				new Object[] { hkObjOrder.getHkObjId(), hkObjOrder.getKind(),
						hkObjOrder.getCityId() }) > 0) {
			return true;
		}
		return false;
	}

	public boolean createHkObjOrder(HkObjOrder hkObjOrder)
			throws NoEnoughMoneyException, SmallerThanMinMoneyException {
		Company company = this.companyService.getCompany(hkObjOrder
				.getHkObjId());
		if (isDuplicate(hkObjOrder)) {
		...
		HkObjOrderDef hkObjOrderDef = this.orderDefService.getHkObjOrderDef(
				hkObjOrder.getKind(), company.getKindId(), hkObjOrder
						.getCityId());
		int minmoney = 0;
		// 设置最低竞排价格
		if (hkObjOrderDef == null) {
		...
		if (hkObjOrder.getMoney() < minmoney) {
			SmallerThanMinMoneyException e = new SmallerThanMinMoneyException(
					"must bigger than minmoney [ " + hkObjOrder.getMoney()
							+ " , " + minmoney + " ]");
			e.setMinmoney(minmoney);
			throw e;
		}
		int res = hkObjOrder.getResultMoney();// 计算总价
		if (company.getMoney() < res) {// 余额不足扣除当前则异常抛出
			throw new NoEnoughMoneyException("no enough money [ "
					+ company.getMoney() + " , " + res + " ]");
		}
		this.companyService.addMoney(hkObjOrder.getHkObjId(), -hkObjOrder
				.getMoney());
		hkObjOrder.setUtime(new Date());
		Query query = manager.createQuery();
		query.addField("hkobjid", hkObjOrder.getHkObjId());
	...
	public int countHkObjOrder(byte stopflg) {
		Query query = manager.createQuery();
		return query.count(HkObjOrder.class, "stopflg=?",
				new Object[] { stopflg });
	}

	public List<HkObjOrder> getHkObjOrderList(byte stopflg, int begin, int size) {
		Query query = manager.createQuery();
		return query.listEx(HkObjOrder.class, "stopflg=?",
				new Object[] { stopflg }, begin, size);
	}
	...
	public List<HkObjOrder> getHkObjOrderListForOrder(byte kind, int cityId,
			int begin, int size) {
		Query query = manager.createQuery();
		return query.listEx(HkObjOrder.class,
				"kind=? and cityid=? and stopflg=?", new Object[] { kind,
						cityId, HkObjOrder.STOPFLG_N },
				"money desc,utime desc", begin, size);
	}

	public List<HkObjOrder> getHkObjOrderListByObjId(long objId, int begin,
			int size) {
		Query query = manager.createQuery();
		return query.listEx(HkObjOrder.class, "hkobjid=?",
				new Object[] { objId }, "oid desc", begin, size);
	}

	public List<HkObjOrder> getHkObjOrderListByUserId(long userId, int cityId,
			int begin, int size) {
		Query query = manager.createQuery();
		if (cityId < 0) {
			return query.listEx(HkObjOrder.class, "userid=?",
					new Object[] { userId }, "oid desc", begin, size);
		}
		return query.listEx(HkObjOrder.class, "userid=? and cityid=?",
				new Object[] { userId, cityId }, "oid desc", begin, size);
	}

	public List<HkObjOrder> getHkObjOrderListByObjId(long objId, int cityId,
			int begin, int size) {
		Query query = manager.createQuery();
		if (cityId < 0) {
			return query.listEx(HkObjOrder.class, "hkobjid=?",
		...
		return query.listEx(HkObjOrder.class, "hkobjid=? and cityid=?",
	...
	public void calculate(HkObjOrder hkObjOrder) {
		...
		Company company = this.companyService.getCompany(hkObjOrder
				.getHkObjId());
		if (hkObjOrder.getMoney() > company.getMoney()) {// 余额不足
			return;
		}
		this.companyService.addMoney(hkObjOrder.getHkObjId(), -hkObjOrder
				.getMoney());
	...
	public HkObjOrder getHkObjOrder(long objId, byte kind, int cityId) {
		Query query = manager.createQuery();
		return query.getObjectEx(HkObjOrder.class,
				"hkobjid=? and kind=? and cityid=?", new Object[] { objId,
						kind, cityId });
	}
}