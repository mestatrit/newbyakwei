...
public class CmpActorCmtProcessor {
	...
	public void createCmpActorCmt(CmpActorCmt cmpActorCmt,
			CmpActorScore cmpActorScore) {
		this.cmpActorCmtService.createCmpActorCmt(cmpActorCmt);
		this.cmpActorCmtService.updateCmpActorCmtScore(
				cmpActorCmt.getActorId(), cmpActorCmt.getUserId(),
				cmpActorScore.getScore());
	...
	public void saveCmpActorScore(CmpActorScore cmpActorScore) {
		...
		this.cmpActorCmtService.updateCmpActorCmtScore(cmpActorScore
				.getActorId(), cmpActorScore.getUserId(), cmpActorScore
				.getScore());
		// 计算工作人员分数
		int actor_score = this.cmpActorScoreService.sumScoreByActorId(actorId);
		int actor_count = this.cmpActorScoreService
				.countCmpActorScoreByActorId(actorId);
		...
		// 计算店铺分数
		int cmp_score = this.cmpActorService
				.sumCmpActorScoreByCompanyId(companyId);
		int cmp_count = this.cmpActorService
				.sumCmpActorScoreUserNumByCompanyId(companyId);
	...
	public void deleteCmpActorCmt(CmpActorCmt cmpActorCmt) {
		...
		this.cmpActorScoreService.deleteCmpActorScoreByActorIdAndUserId(
				cmpActorCmt.getActorId(), cmpActorCmt.getUserId());
	}

	public List<CmpActorCmt> getCmpActorCmtListByCompanyId(long companyId,
			boolean buildUser, boolean buildActor, int begin, int size) {
		List<CmpActorCmt> list = this.cmpActorCmtService
				.getCmpActorCmtListByCompanyId(companyId, begin, size);
		if (buildUser) {
			this.buildUser(list);
		}
		if (buildActor) {
	...
	public List<CmpActorCmt> getCmpActorCmtListByActorId(long actorId,
			boolean buildUser, boolean buildActor, int begin, int size) {
		List<CmpActorCmt> list = this.cmpActorCmtService
				.getCmpActorCmtListByActorId(actorId, begin, size);
		if (buildUser) {
			this.buildUser(list);
		}
		if (buildActor) {
	...
	public List<CmpActorCmt> getCmpActorCmtList(boolean buildUser,
			boolean buildActor, int begin, int size) {
		List<CmpActorCmt> list = this.cmpActorCmtService.getCmpActorCmtList(
				begin, size);
		if (buildUser) {
			this.buildUser(list);
		}
