...
public class CmpOtherInfo {
	...
	public String getBeginTime() {
		if (this.durationdata != null) {
			Map<String, String> map = DataUtil
					.getMapFromJson(this.durationdata);
			return map.get("b");
		}
		return null;
	}
	...
	public int validate() {
		if (this.getEndTime() != null && this.getBeginTime() != null) {
			if (!this.getBeginTime().endsWith("00")
					&& !this.getBeginTime().endsWith("30")) {
				return Err.CMPOTHERINFO_DURATION_ERROR;
			}
			if (!this.getEndTime().endsWith("00")
					&& !this.getEndTime().endsWith("30")) {
				return Err.CMPOTHERINFO_DURATION_ERROR;
			}
		}
		return Err.SUCCESS;
