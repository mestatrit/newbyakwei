...
public interface CmpActorService {
	 ...
	 *         2010-7-21
	 */
	List<CmpActor> getCmpActorListByCompanyId(long companyId, long roleId,
			String name, int begin, int size);

	Map<Long, CmpActor> getCmpActorMapInId(List<Long> idList);

	Map<Long, CmpActorRole> getCmpActorRoleMapByCompanyIdAndInId(
			List<Long> idList);
	 ...
	 *         2010-8-20
	 */
	List<CmpActor> getCmpActorListByCompanyIdForCanReserve(long companyId,
			int begin, int size);
