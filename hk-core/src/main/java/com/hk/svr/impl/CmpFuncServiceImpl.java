...
public class CmpFuncServiceImpl implements CmpFuncService {
	...
	public void createCmpFuncRef(CmpFuncRef cmpFuncRef) {
		...
		if (query.count(CmpFuncRef.class, "companyid=? and funcoid=?",
				new Object[] { cmpFuncRef.getCompanyId(),
						cmpFuncRef.getFuncoid() }) > 0) {
			return;
		}
	...
	public void deleteCmpFuncRef(long companyId, long oid) {
		Query query = this.manager.createQuery();
		query.delete(CmpFuncRef.class, "companyid=? and oid=?", new Object[] {
				companyId, oid });
	}

	public CmpFuncRef getCmpFuncRef(long companyId, long oid) {
		Query query = this.manager.createQuery();
		return query.getObjectEx(CmpFuncRef.class, "companyid=? and oid=?",
				new Object[] { companyId, oid });
	}

	public List<CmpFuncRef> getCmpFuncRefListByCompanyId(long companyId) {
		Query query = this.manager.createQuery();
		return query.listEx(CmpFuncRef.class, "companyid=?",
