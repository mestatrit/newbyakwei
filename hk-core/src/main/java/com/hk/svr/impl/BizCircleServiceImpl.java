...
public class BizCircleServiceImpl implements BizCircleService {
	...
	public List<BizCircle> getBizCircleList(String name, int cityId,
			int provinceId) {
		Query query = this.manager.createQuery();
		List<Object> olist = new ArrayList<Object>();
		StringBuilder sql = new StringBuilder(
				"select * from bizcircle where 1=1");
	...
	public List<BizCircle> getBizCircleListForHasCmp(int cityId) {
		...
		List<Object> olist = new ArrayList<Object>();
		StringBuilder sql = new StringBuilder(
				"select * from bizcircle where cmpcount>0");
		if (cityId > 0) {
			sql.append(" and cityid=?");
			olist.add(cityId);
		}
	...
	public void createCompanyBizCircle(long companyId, int circleId) {
		...
		query.where("companyid=? and circleid=?").setParam(companyId).setParam(
				circleId);
		if (query.count() == 0) {
	...
	public void updateBizCircleCmpCount(int circleId) {
		Query query = manager.createQuery();
		int count = query.count(CompanyBizCircle.class, "circleid=?",
				new Object[] { circleId });
		query.addField("cmpcount", count);
		query.updateById(BizCircle.class, circleId);
	}

	public List<Company> getCompanyList(int circleId, int kindId, int parentId,
			int begin, int size) {
		List<Object> olist = new ArrayList<Object>();
		StringBuilder sql = new StringBuilder(
				"select c.* from companybizcircle bc,company c where bc.circleid=? and bc.companyid=c.companyid");
		olist.add(circleId);
		if (kindId > 0) {
	...
	public int countCompany(int circleId, int kindId, int parentId, int cityId) {
		List<Object> olist = new ArrayList<Object>();
		StringBuilder sql = new StringBuilder(
				"select count(*) from companybizcircle bc,company c where bc.circleid=? and bc.companyid=c.companyid");
		olist.add(circleId);
		if (kindId > 0) {
			sql.append(" and c.kindid=?");
			olist.add(kindId);
	...
	public void updatecitydata() {
		...
		for (BizCircle o : list) {
			if (o.getCityId() == 0 && o.getProvinceId() != 0) {
				...
				if (province != null) {
					String name = DataUtil.filterZoneName(province
							.getProvince());
