...
public class OtherApiConsumer {
	...
	public void processMessage(String value) {
		...
		if (jmsMsg.getHead().equals(
				JmsMsg.HEAD_OTHER_API_SINAFOLLOW_CREATE_FOLLOW)) {
			this.processSinaFollow(jmsMsg.getBody());
			return;
		}
		if (jmsMsg.getHead().equals(
				JmsMsg.HEAD_OTHER_API_CREATE_ITEM_CMT_TO_STATUS)) {
		...
		if (jmsMsg.getHead().equals(
				JmsMsg.HEAD_OTHER_API_CREATE_ANSWER_TO_STATUS)) {
	...
	private void processShareItem(String body) {
		...
		String content = null;
		String item_url = "http://" + map.get(JsonKey.SERVER_NAME)
				+ map.get(JsonKey.CONTEXTPATH) + "/tb/item?itemid="
				+ itemid_str;
		if (tbItem.getClick_url() != null && tbItem.getCommission() > 0) {
			content = ResourceConfig.getText(
					"ehk.buyitem.sina.contentwithclick_url", tbItem.getTitle(),
					DataUtil.fmtDouble(tbItem.getUserYouHuiPrice(), "#.#"),
					item_url);
		}
		else {
			content = ResourceConfig.getText("ehk.buyitem.sina.content", tbItem
					.getTitle(), item_url);
		}
		ImageItem imageItem = null;
		if (tbItem.getPic_url() != null) {
			try {
				byte[] imgbyte = this.httpUtil.getByteArrayResult(tbItem
						.getPic_url());
		...
		Tb_User_Api userApi = this.tbUserService.getTb_User_Api(userid,
				Tb_User_Api.REG_SOURCE_SINA);
		if (userApi == null) {
			return;
		}
		try {
			SinaUtil.updateStatus(userApi.getAccess_token(), userApi
					.getToken_secret(), content, imageItem);
		}
	...
	private void processCreateAnswerStatusToSina(String body) {
		...
		String content = DataUtil.limitTextRow(tbAnswer.getContent(), 80)
				+ " http://" + server_name + contextpath + "/tb/ask?aid="
				+ tbAnswer.getAid();
		Tb_User_Api userApi = this.tbUserService.getTb_User_Api(tbAnswer
	...
	private void processCreateAskStatusToSina(String body) {
		...
		String content = DataUtil.toTextRow(tbAsk.getTitle()) + " http://"
				+ server_name + contextpath + "/tb/ask?aid=" + aid;
		Tb_User_Api userApi = this.tbUserService.getTb_User_Api(tbAsk
				.getUserid(), Tb_User_Api.REG_SOURCE_SINA);
		try {
			SinaUtil.updateStatus(userApi.getAccess_token(), userApi
					.getToken_secret(), content);
	...
	private void processCreateCmtStatusToSina(String body) {
		...
		String content = DataUtil.limitHtmlRow(tbItemCmt.getContent(), 80)
				+ "http://" + server_name + contextpath + "/tb/item?itemid="
				+ tbItemCmt.getItemid();
		Tb_User_Api userApi = this.tbUserService.getTb_User_Api(tbItemCmt
				.getUserid(), Tb_User_Api.REG_SOURCE_SINA);
		try {
			SinaUtil.updateStatus(userApi.getAccess_token(), userApi
					.getToken_secret(), content);
	...
	private void processSinaFollow(String body) {
		...
		Tb_User_Api friendApi = this.tbUserService.getTb_User_Api(friendid,
				Tb_User_Api.REG_SOURCE_SINA);
		if (friendApi == null) {
			return;
		}
		try {
			SinaUtil.followUser(userApi.getAccess_token(), userApi
					.getToken_secret(), friendApi.getUid());
		}
		catch (WeiboException e) {
