...
<hk:wap title="爆破日志 - 火酷" rm="false" style="/page/css/b.css">
	...
	<c:if test="${fn:length(bomblabavoList)>0}">
		<c:forEach var="b" items="${bomblabavoList}" varStatus="idx">
			...
			<div class="hang ${clazz_var}">
				<hk:a href="/home.do?userId=${b.bombLaba.laba.userId}">${b.bombLaba.laba.user.nickName}</hk:a> 
				${b.labaVo.content } 
				<span class="ruo"><fmt:formatDate value="${b.bombLaba.createTime}" pattern="yy-MM-dd HH:mm"/></span> 
				<hk:a clazz="s" href="/bomb/bomb_recoverlaba.do?sysId=${b.bombLaba.sysId}&userId=${userId }" page="true">恢复</hk:a>
			</div>
		</c:forEach>
		<c:if test="${userId>0}">
