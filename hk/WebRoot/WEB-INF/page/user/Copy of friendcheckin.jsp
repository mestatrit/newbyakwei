...
<hk:wap title="火酷" rm="false" bodyId="thepage">
	...
	<c:if test="${fn:length(citylist)>0}">
		<c:forEach var="log" items="${citylist}" varStatus="idx"><c:if test="${idx.index%2==0}"><c:set var="clazz_var" value="odd" /></c:if><c:if test="${idx.index%2!=0}"><c:set var="clazz_var" value="even" /></c:if>
	...
	<c:if test="${fn:length(othercitylist)>0}">
		<div class="hang even">其他地区</div>
		<c:forEach var="log" items="${othercitylist}" varStatus="idx"><c:if test="${idx.index%2==0}"><c:set var="clazz_var" value="odd" /></c:if><c:if test="${idx.index%2!=0}"><c:set var="clazz_var" value="even" /></c:if>
			<div class="hang ${clazz_var }">
				<hk:a href="/home.do?userId=${log.userId}">${log.user.nickName }</hk:a>
				在
				<hk:a href="/e/cmp.do?companyId=${log.companyId}">${log.company.name}</hk:a>
