...
<hk:wap title="往期宝箱 - 火酷" rm="false" bodyId="thepage">
	<jsp:include page="../../inc/top.jsp"></jsp:include>
	<div class="hang even">往期宝箱</div>
	<c:if test="${fn:length(list)>0}">
		<table class="list" cellpadding="0" cellspacing="0"><tbody>
			<c:forEach var="b" items="${list}" varStatus="idx"><c:if test="${idx.index%2==0}"><c:set var="clazz_var" value="odd" /></c:if><c:if test="${idx.index%2!=0}"><c:set var="clazz_var" value="even" /></c:if>
				<tr class="${clazz_var }"><td><hk:a href="/box/op/op_getbox.do?boxId=${b.boxId}&t=3" page="true">${b.name}</hk:a></td></tr>
			</c:forEach>
		</tbody></table>
		<hk:simplepage href="/box/op/op_overboxlist.do?name=${enc_name}" />
	</c:if>
	<c:if test="${fn:length(list)==0}">没有数据显示</c:if>
	<div class="hang">
		<hk:form method="get" action="/box/op/op_overboxlist.do">
