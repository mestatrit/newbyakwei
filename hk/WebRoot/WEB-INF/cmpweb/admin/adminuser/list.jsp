		...
		<div class="mod_content">
			<div class="divrow">
				<form action="<%=path %>/epp/mgr/web/adminuser_find.do">
					...
					<hk:submit clazz="btn" value="添加管理员"/>
				</form>
			</div>
					...
					<c:forEach var="n" items="${list}">
						<div class="divrow bdtm" onmouseover="this.className='divrow bdtm bg2'" onmouseout="this.className='divrow bdtm'">
							...
							<div class="f_l" style="width:80px;padding-left: 20px">
								<a href="javascript:delcmpadminuser(${n.oid })">删除</a>
							</div>
							<div class="clr"></div>
						</div>
					</c:forEach>
			...
			<c:if test="${fn:length(list)==0}">
				<div class="nodata">还没有添加任何管理员</div>
			</c:if>
		</div>
		</div>
	</div>
...
function delcmpadminuser(oid){
	if(window.confirm('确实要删除？')){
		$.ajax({
			type:"POST",
			url:"<%=path%>/epp/mgr/web/adminuser_del.do?companyId=${companyId}&oid="+oid,
			cache:false,
	    	dataType:"html",
