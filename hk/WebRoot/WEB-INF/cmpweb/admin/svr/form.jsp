	...
	<table class="nt all" cellpadding="0" cellspacing="0">
			...
			<td>
				<c:if test="${cmpSvr!=null}">
					<c:if test="${cmpSvr.photosetId>0}">
						...
						${cmpPhotoSet.name } <a href="javascript:selset(${svrId })">更换图集</a>
					</c:if>
					<c:if test="${cmpSvr.photosetId==0}">
						<a href="javascript:selset(${svrId })">选择图集</a>
					</c:if>
				</c:if>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				<a href="<%=path %>/epp/web/op/webadmin/svr.do?companyId=${companyId}">返回</a>
			</td>
		</tr>
	</table>
</form>
...
function selset(svrId){
	tourl("<%=path %>/epp/web/op/webadmin/info_selphotosetforsvr.do?companyId=${companyId}&svrId="+svrId+"&return_url="+encodeLocalURL());
}
</script>