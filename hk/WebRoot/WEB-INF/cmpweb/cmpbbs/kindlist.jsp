...
<c:set var="html_title" scope="request"><hk:data key="epp.bbs"/> - ${o.name}</c:set>
<c:set var="html_body_content" scope="request">
<div class="hcenter" style="">
<c:forEach var="kindvo" items="${volist}">
	<div class="mod"><%EppViewUtil.loadCmpNavTop(request); %>
		<div class="mod_title">
			<a href="<%=path %>/epp/web/cmpbbs_kind.do?companyId=${companyId}&kindId=${kindvo.cmpBbsKind.kindId}">${kindvo.cmpBbsKind.name }</a>
		</div>
		<div class="mod_content">
			<div class="divrow">
				<input type="button" class="btn" value="<hk:data key="epp.createbbs"/>" onclick="tourl('<%=path %>/epp/web/cmpbbs_create.do?companyId=${companyId}&kindId=${kindvo.cmpBbsKind.kindId}&return_url=${return_url }')"/>
			</div>
			<c:if test="${fn:length(kindvo.bbsList)==0}">
				<div class="nodata"><hk:data key="epp.bbs.nodata"/></div>
			</c:if>
			<c:if test="${fn:length(kindvo.bbsList)>0}">
				<ul class="bbslist">
					<c:forEach var="bbs" items="${kindvo.bbsList}" varStatus="idx">
			...
			<c:if test="${kindvo.hasMore}">
				<div class="divrow">
					<a href="<%=path %>/epp/web/cmpbbs_kind.do?companyId=${companyId}&kindId=${kinkvo.cmpBbsKind.kindId}"><hk:data key="epp.more"/></a>
				</div>
			</c:if>
		</div>
	</div>
