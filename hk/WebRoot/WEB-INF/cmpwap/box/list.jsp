...
<c:if test="${fn:length(list)>0}">
	...
	<div class="row ${clazz_var }">
		<a href="<%=path %>/epp/web/box_wapview.do?companyId=${companyId}&boxId=${box.boxId}&navId=${navId}">${box.name }</a>
	</div>
	</c:forEach>
</c:if>
<div class="row">
	<hk:simplepage2 href="/epp/web/box_wap.do?companyId=${companyId}&navId=${navId}"/>
</div>
