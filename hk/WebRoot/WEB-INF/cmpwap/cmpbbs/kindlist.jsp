...
<c:if test="${fn:length(list)>0}">
	<c:forEach var="kind" items="${list}" varStatus="idx"><c:if test="${idx.index%2==0}"><c:set var="clazz_var" value="odd" /></c:if><c:if test="${idx.index%2!=0}"><c:set var="clazz_var" value="even" /></c:if>
	<div class="row ${clazz_var }">
		<a href="<%=path %>/epp/web/cmpbbs_wapkind.do?companyId=${companyId}&kindId=${kind.kindId}&navId=${navId}">${kind.name }</a>
	</div>
	</c:forEach>
</c:if>
