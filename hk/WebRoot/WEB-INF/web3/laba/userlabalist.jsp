...
<c:set var="html_title" scope="request">
	<hk:data key="view.user.laba.title" arg0="${user.nickName}" />
</c:set>
				...
				<div class="mid_con">
					<div class="mod">
						<c:set var="nav_2_short_content" scope="request">
							<a href="<%=path%>/laba_userlaba.do?userId=${userId }" class="nav-a"><hk:data key="view.user.laba.title" arg0="${user.nickName}" /></a>
						</c:set>
						<jsp:include page="../inc/nav-2-short.jsp"></jsp:include>
					</div>
					<div class="mod">
						<c:set var="page_url" scope="request"><%=path%>/laba_userlaba.do?userId=${userId}</c:set>
