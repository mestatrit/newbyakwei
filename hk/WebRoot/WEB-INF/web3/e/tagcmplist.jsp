...
<c:set var="html_title" scope="request">${companyTag.name} - 火酷网</c:set>
<c:set var="meta_value" scope="request">
<meta name="keywords" content="${companyTag.name },火酷网,足迹"/>
<meta name="description" content="${companyTag.name },火酷网,足迹"/>
</c:set>
	...
	<div class="mod_primary">
		<c:set var="nav_2_path_content" scope="request">
			<ul>
				...
				<li>
					<a class="nav-a" href="<%=path %>/cmp_bzcmplist.do?tagId=${tagId }&parentId=${parentId }&kindId=${kindId }&${url_add }">${companyTag.name}</a>
				</li>
			</ul>
		</c:set>
		<jsp:include page="../inc/nav-2.jsp"></jsp:include>
		<div class="mod_primary_l">
			<c:set var="page_url" scope="request">/cmp_tagmplist.do?tagId=${tagId}&parentId=${parentId }&kindId=${kindId}&pcityId=${pcityId}</c:set>
