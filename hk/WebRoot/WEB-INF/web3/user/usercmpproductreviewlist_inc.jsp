...
<c:set var="iteratorreview">
	<c:forEach var="reviewvo" items="${cmpproductreviewvolist}">
		<li class="review" id="review${reviewvo.cmpProductReview.labaId }" onmouseout="this.className='review';" onmouseover="this.className='review bg3';">
			<table cellpadding="0" cellspacing="0">
				<tr>
					<td class="head">
						<a href="<%=path%>/product.do?pid=${reviewvo.cmpProductReview.productId }"><img src="${reviewvo.cmpProductReview.cmpProduct.head60 }" /><br />${reviewvo.cmpProductReview.cmpProduct.name }</a>
					</td>
					<td class="review-body">
						...
						<div class="clr"></div>
						${reviewvo.content} 
						<div align="right" class="text_12">
							<c:if test="${loginUser.userId==reviewvo.cmpProductReview.userId}">
								<a id="editcmpproductreview${reviewvo.cmpProductReview.labaId }" href="javascript:toeditcmpproductreview(${reviewvo.cmpProductReview.labaId })" class="split-r">修改</a> 
							</c:if>
...
function todelcmpproductreview(labaId){
	if(window.confirm("确实要删除？")){
		tourl("<%=path%>/op/product_delreview.do?labaId="+labaId+"&return_url="+encodeLocalURL());
	}
}
function toeditcmpproductreview(labaId){
	//tmp_data=getHtml('createcmpproductreviewcon');
	//setHtml('createcmpproductreviewcon','');
