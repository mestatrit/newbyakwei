...
<c:set var="html_title" scope="request">
	<hk:data key="view.user.mgr.chagenmobile" />
</c:set>
<c:set var="mgr_content" scope="request">
	<div class="text_14">
		<c:if test="${userOtherInfo.mobileAlreadyBind}">
			<h3 class="title3"><hk:data key="view.userotherinfo.currentmobileforbind" arg0="${userOtherInfo.mobile}" /></h3>
		</c:if>
		<h3 class="title3">更换手机号</h3>
		您的手机验证码是
		<span id="code1" class="yzm">yz${ran.randvalue}</span>(本验证码10分钟内有效).
		<br />
		请通过手机发送
		<span id="code2" class="yzm">yz${ran.randvalue}</span>到1066916025完成本帐号的手机号绑定,10分钟之内有效.发送完毕后请耐心等待1分钟,1分钟后再次点击下面发送完毕链接.
		<br />
		不操作不会对已经绑定的手机号有任何影像
		<br />
