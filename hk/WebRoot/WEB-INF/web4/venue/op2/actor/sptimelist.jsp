...
<a class="more" href="<%=path %>/h4/op/venue/actor.do?companyId=${companyId}&actorId=${actorId}">返回</a>
</div>
<div class="mod_content">
	<div class="divrow">
		<input value="申请假期" type="button" class="btn split-r" onclick="tourl('<%=path %>/h4/op/venue/actor_createactorsptime.do?companyId=${companyId}&actorId=${actorId }')"/>
</div>
...
function delsptime(oid){
	if(window.confirm('确实要删除？')){
		$.ajax({
			type:"POST",
			url:"<%=path%>/h4/op/venue/actor_delactorsptime.do?companyId=${companyId}&oid="+oid,
			cache:false,
	    	dataType:"html",
...
function updatesptime(oid){
	tourl("<%=path%>/h4/op/venue/actor_updateactorsptime.do?companyId=${companyId}&actorId=${actorId}&oid="+oid+"&return_url="+encodeLocalURL());
}
</script>
