...
<tr>
	<td style="font-size: 16px;">
		<hk:data key="view2.tips.create_infotip"/>
	</td>
</tr>
</table>
<br/>
...
<div id="res">
	<div>
	<h1>${loginUser.pcity.name }</h1>
	</div>
	<c:if test="${fn:length(list)==0}">
		<hk:data key="view2.data_not_found"/>
	</c:if>
	<c:if test="${fn:length(list)>0}">
	...
	<div>
		<h1><hk:data key="view2.searchvenue_on_other_city"/></h1>
	</div>
	<c:if test="${fn:length(otherlist)==0}">
		<hk:data key="view2.data_not_found"/>
	</c:if>
	<div style="padding-top: 20px">
	<c:if test="${fn:length(otherlist)>0 || fn:length(list)>0}">
		<hk:data key="view2.alldata_was_wrong"/>
	</c:if>
		<a class="more2" href="/venue/create?name=${enc_name }"><hk:data key="view2.create_new_venue"/>：${name }</a>
	</div>
</div>
</c:if>
</div>
