...
<c:if test="${fn:length(productlist)==0}"><hk:data key="nodataview"/></c:if>
<c:if test="${fn:length(productlist)>0}">
	<c:forEach var="p" items="${productlist}" varStatus="idx"><c:if test="${idx.index%2==0}"><c:set var="clazz_var" value="odd" /></c:if><c:if test="${idx.index%2!=0}"><c:set var="clazz_var" value="even" /></c:if>
