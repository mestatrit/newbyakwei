...
<div class="nav2">与<hk:a href="/union/home.do?userId=${main.user2.userId}&uid=${uid }">${main.user2.nickName}</hk:a>|<hk:a href="/union/op/msg/pvtlist.do?uid=${uid }">所有对话录</hk:a>|<hk:a href="/union/op/msg/send_tosend.do?uid=${uid }">发送私信</hk:a></div>
<c:if test="${fn:length(volist)>0}">
			...
			<c:forEach var="vo" items="${volist}" varStatus="idx"><c:if test="${idx.index%2==0}"><c:set var="clazz_var" value="odd" /></c:if><c:if test="${idx.index%2!=0}"><c:set var="clazz_var" value="even" /></c:if>
				<tr class="${clazz_var}">
					...
					<td>
						...
						${vo.pvtChat.msg } 
						<hk:a href="/union/op/msg/pvt_delchat.do?chatId=${vo.pvtChat.chatId}&mainId=${mainId }&uid=${uid }" page="true">删</hk:a>
					</td>
				</tr>
			</c:forEach>
		</tbody></table>
		<hk:simplepage2 clazz="page" href="/union/op/msg/chat.do?mainId=${main.mainId}&uid=${uid }" />
	...
	<div class="row"><hk:a href="/union/op/msg/chat.do?mainId=${main.mainId}&uid=${uid }">返回</hk:a></div>
